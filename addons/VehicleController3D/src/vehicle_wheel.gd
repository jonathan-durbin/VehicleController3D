...
func _handle_acceleration() -> void:
	...
	# TODO: Consider making this an export
...
func _handle_traction() -> void:
	...
	# Another option to calculate x_traction:
	# x_traction = parent.grip_curve_map[front_back].sample(clampf(
	# 	absf(x_speed)/parent.settings.max_wheel_x_speed,
	# 	0.0, 1.0
	# ))

	# Manually overriding x_traction with a handbrake feels wrong somehow...
	...
	var speed_change: float = -x_speed * x_traction
	total_forces.x = speed_change * _get_wheel_weight()

	var force_vector: Vector3 = total_forces.x * steer_side_dir
	# Might want to consider applying the force at the contact point
	var force_offset_pos: Vector3 = wheel_mesh.global_position - parent.global_position
...
func initialize() -> void:
	...
	collision_mask = parent.ground_layer
